...
class EquityGapTrajectory:
    ...
    def load_data(self):
        ...
        self.sites = df.groupby('key_plot_id').agg({
        ...
        self.state_svi = svi.groupby('ST_ABBR').apply(
            lambda x: pd.Series({
    ...
    def assign_svi_quartiles(self):
        ...
        print(self.state_svi.groupby('svi_quartile').agg({
            'STATE': 'count',
            'total_population': 'sum',
            'RPL_THEMES': ['min', 'max', 'mean']
        }))
    ...
    def calculate_quarterly_expansion(self):
        ...
        quarterly = sites_with_svi.groupby(['quarter', 'svi_quartile']).size().reset_index(name='new_sites')
        ...
        quarterly['pct_growth'] = quarterly.groupby('svi_quartile')['cumulative_sites'].pct_change() * 100
    ...
    def calculate_expansion_stats(self):
        ...
        stats = sites_with_svi.groupby('svi_quartile').agg({
            'key_plot_id': 'count',
        ...
        state_pop = self.state_svi.groupby(
            self.state_svi['ST_ABBR'].map(
                dict(zip(self.state_svi['ST_ABBR'],
                        pd.qcut(self.state_svi['RPL_THEMES'], q=4,
                               labels=['Q1 (Lowest SVI)', 'Q2 (Low-Mid SVI)',
                                      'Q3 (Mid-High SVI)', 'Q4 (Highest SVI)'])))
            )
        )['total_population'].sum()
    ...
    def plot_cumulative_expansion(self, save_path: str = None):
        ...
        for quartile in self.quarterly_data['svi_quartile'].unique():
            ...
            ax.plot(data['date'], data['cumulative_sites'],
                   marker='o', label=quartile, color=colors.get(quartile),
                   linewidth=2.5, markersize=4)

        ax.set_xlabel('Date', fontsize=12, fontweight='bold')
        ax.set_ylabel('Cumulative Monitoring Sites', fontsize=12, fontweight='bold')
        ax.set_title('Wastewater Surveillance Expansion by State SVI Quartile\n' +
                    'Cumulative Sites Over Time', fontsize=14, fontweight='bold', pad=20)
    ...
    def plot_quarterly_new_sites(self, save_path: str = None):
        ...
        ax.set_title('Quarterly Expansion of Wastewater Surveillance by State SVI Quartile\n' +
                    'New Sites per Quarter', fontsize=14, fontweight='bold', pad=20)
    ...
    def plot_gap_trajectory(self, save_path: str = None):
        ...
        ax1.plot(gap_metrics['date'], gap_metrics['cumulative_gap_ratio'],
        ...
        ax1.set_title('Equity Gap Trajectory: Cumulative Sites Ratio (Q1 vs Q4)\n' +
        ...
        gap_metrics['new_sites_gap_smooth'] = gap_metrics['new_sites_gap_ratio'].rolling(
            window=4, min_periods=1
        ).mean()

        ax2.plot(gap_metrics['date'], gap_metrics['new_sites_gap_smooth'],
                marker='o', color='#3498db', linewidth=2.5, markersize=6)
        ...
        ax2.set_title('Equity Gap Trajectory: New Sites Ratio (4-Quarter Moving Average)\n' +
    ...
    def plot_expansion_rates(self, save_path: str = None):
        ...
        expansion_rates = sites_with_svi.groupby(['period', 'svi_quartile']).size().reset_index(name='sites')
        ...
        ax.set_title('Wastewater Surveillance Expansion by Time Period and SVI Quartile\n' +
    ...
    def generate_summary_report(self):
        ...
        report.append(f"Time period: {self.sites['first_sample_date'].min().strftime('%Y-%m-%d')} to " +
        ...
        report.append(f"   - Gap Ratio: {current_gap:.2f}x (Q1 has {current_gap:.2f}x more sites than Q4)")
        ...
        report.append(f"   - Q1 states added: {q1_new} sites ({stats.loc['Q1 (Lowest SVI)', 'pct_new_2024']:.1f}% of their total)")
        report.append(f"   - Q4 states added: {q4_new} sites ({stats.loc['Q4 (Highest SVI)', 'pct_new_2024']:.1f}% of their total)")
    ...
    def run_full_analysis(self, output_dir: str = None):
        ...
        self.plot_cumulative_expansion(
            save_path=output_dir / 'gap_trajectory_cumulative.png'
        )

        self.plot_quarterly_new_sites(
            save_path=output_dir / 'gap_trajectory_quarterly_new.png'
        )

        self.plot_gap_trajectory(
            save_path=output_dir / 'gap_trajectory_ratio.png'
        )

        self.plot_expansion_rates(
            save_path=output_dir / 'gap_trajectory_expansion_rates.png'
        )
...
def main():
    ...
    # Run analysis
    analyzer = EquityGapTrajectory(
        wastewater_path=str(wastewater_path),
