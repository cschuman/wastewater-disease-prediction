...
def create_analysis_dataset(data_dir: Path = Path("data")) -> pd.DataFrame:
    ...
    print(
        f"  Counties with RUCC codes: {df['rucc_2023'].notna().sum()} ({df['rucc_2023'].notna().mean()*100:.1f}%)"
    )
    ...
    df["svi_quartile"] = pd.qcut(
        df["svi_overall"], q=4, labels=["Q1 (Low)", "Q2", "Q3", "Q4 (High)"], duplicates="drop"
    )

    print(f"\nFinal analysis dataset: {len(df)} counties")
    print(
        f"  With monitoring: {df['has_monitoring'].sum()} ({df['has_monitoring'].mean()*100:.1f}%)"
    )
...
def analyze_svi_urbanization_correlation(df: pd.DataFrame) -> dict:
    ...
    groups = [
        analysis_df[analysis_df["urbanization"] == cat]["svi_overall"].dropna()
...
def analyze_baseline_svi_effect(df: pd.DataFrame) -> dict:
    ...
    if baseline_model.pvalues["svi_overall"] < 0.05:
        ...
        print(
            f"    Effect size: {abs(baseline_model.params['svi_overall']):.2f} sites per 100k per 1-unit SVI increase"
        )
    else:
...
def assess_confounding(baseline_results: dict, controlled_results: dict) -> dict:
    ...
    print(
        f"  Change:                    {controlled_coef - baseline_coef:.3f} ({pct_change:+.1f}%)"
    )
    ...
    print(
        f"  Baseline p-value:          {baseline_results['baseline_p']:.4f} {'***' if baseline_sig else ''}"
    )
    print(
        f"  Controlled p-value:        {controlled_results['model1_svi_p']:.4f} {'***' if controlled_sig else ''}"
    )
...
def run_full_confounding_analysis(data_dir: Path = Path("data")) -> dict:
    ...
    all_results = {"n_counties": len(df), "n_monitored": df["has_monitoring"].sum()}
