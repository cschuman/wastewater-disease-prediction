...
def calculate_hosp_ww_ratios(df: pd.DataFrame, lag_weeks: int = 1) -> pd.DataFrame:
    ...
    for state in df["state"].unique():
        ...
        slope, intercept, r_value, p_value, std_err = stats.linregress(
            state_df["ww_lagged"], state_df["covid_hosp"]
        )
...
def analyze_equity_patterns(ratios_df: pd.DataFrame, svi_df: pd.DataFrame) -> dict:
    """
    ...
    results = {"data": merged, "correlations": {}, "quartile_comparison": {}, "interpretation": []}
