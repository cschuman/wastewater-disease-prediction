...
def analyze_site_expansion(data_dir: Path = Path("data")) -> dict:
    ...
    comparison['pct_change'] = (comparison['change'] / comparison['sites_first'].replace(0, 1) * 100).round(1)
...
def analyze_first_sample_dates(data_dir: Path = Path("data")) -> dict:
    ...
    sites = nwss.groupby('wwtp_id').agg({
...
def difference_in_differences(data_dir: Path = Path("data"), expansion_results: dict = None) -> dict:
    """
    ...
    did_data = nwss.groupby(['state', 'post', 'treatment']).agg({
        'percentile': 'mean',
        'wwtp_id': 'nunique',
    ...
    signal_var = nwss.groupby(['state', 'post']).agg({
        'percentile': 'std'
    }).reset_index()
...
def analyze_covid_surge_moderator(data_dir: Path = Path("data")) -> dict:
    ...
    state_surge = nwss.groupby(['state', 'is_surge']).agg({
        'percentile': ['mean', 'std'],
        'wwtp_id': 'nunique'
    }).reset_index()
    ...
    print(f"  Surge main effect: {interact_model.params['is_surge']:.3f} (p={interact_model.pvalues['is_surge']:.4f})")
    print(f"  SVI*Surge interaction: {interact_model.params['svi_surge']:.3f} (p={interact_model.pvalues['svi_surge']:.4f})")
...
def run_all_natural_experiments(data_dir: Path = Path("data")) -> dict:
    ...
    surge_str = f"{surge_coef:.3f}" if surge_coef is not None and not np.isnan(surge_coef) else "N/A"
