...
def load_county_svi(data_dir: Path = Path("data")) -> pd.DataFrame:
    ...
    svi_clean.columns = [
        ...
        'pop_poverty', 'pop_unemployed', 'pop_no_hs', 'pop_uninsured'
    ]
...
def create_analysis_dataset(data_dir: Path = Path("data")) -> pd.DataFrame:
    ...
    print(f"Counties with monitoring: {merged['has_monitoring'].sum()} ({merged['has_monitoring'].mean()*100:.1f}%)")
...
def analyze_monitoring_coverage_gap(df: pd.DataFrame) -> dict:
    ...
    quartile_stats = df.groupby('svi_quartile').agg({
        'has_monitoring': 'mean',
        'sites_per_100k': 'mean',
        'coverage_pct': 'mean',
        'fips': 'count'
    ...
    groups = [df[df['svi_quartile'] == q]['sites_per_100k'].dropna() for q in df['svi_quartile'].unique()]
...
def analyze_within_state_variation(df: pd.DataFrame) -> dict:
    ...
    state_dummies = pd.get_dummies(analysis_df['state'], prefix='state', drop_first=True).astype(float)
...
def run_mediation_analysis(df: pd.DataFrame) -> dict:
    ...
    print(f"  Coefficient: {model_a.params['svi_overall']:.3f}, p={model_a.pvalues['svi_overall']:.4f}")
    ...
    print(f"  Coefficient: {model_b.params['sites_per_100k']:.4f}, p={model_b.pvalues['sites_per_100k']:.4f}")
    ...
    print(f"  Coefficient: {model_c.params['svi_overall']:.4f}, p={model_c.pvalues['svi_overall']:.4f}")

    # Path c' (Direct effect, controlling for mediator)
    print(f"\nPath c' (Direct: SVI → Signal CV, controlling for Sites):")
    print(f"  Coefficient: {model_b.params['svi_overall']:.4f}, p={model_b.pvalues['svi_overall']:.4f}")
    ...
    results['path_b'] = {'coef': model_b.params['sites_per_100k'], 'p': model_b.pvalues['sites_per_100k']}
    results['path_c'] = {'coef': model_c.params['svi_overall'], 'p': model_c.pvalues['svi_overall']}
    results['path_c_prime'] = {'coef': model_b.params['svi_overall'], 'p': model_b.pvalues['svi_overall']}
