...
class BurninAnalyzer:
    ...
    def load_data(self):
        ...
        print(
            f"First sample dates: {self.df['first_sample_date'].min()} to {self.df['first_sample_date'].max()}"
        )
        ...
        print(
            f"                ({self.df['site_age_weeks'].min():.1f} to {self.df['site_age_weeks'].max():.1f} weeks)"
        )

        return self

    def calculate_signal_metrics_by_age(
        self, age_bins_weeks=[4, 8, 12, 16, 20, 26, 39, 52, 78, 104]
    ):
        """
        ...
        self.df["age_bin"] = pd.cut(
            self.df["site_age_weeks"],
            bins=age_bins,
            labels=[f"{age_bins[i]:.0f}-{age_bins[i+1]:.0f}w" for i in range(len(age_bins) - 1)],
            include_lowest=True,
        )
    ...
    def calculate_per_site_metrics(self, window_weeks=4):
        ...
        for site_id in self.df["key_plot_id"].unique():
            ...
            late_data = site_data[
                site_data["site_age_weeks"] >= max(max_age_weeks - window_weeks, window_weeks)
            ]
            ...
            if len(late_data) > 1 and max_age_weeks > window_weeks * 2:
                ...
                site_info["variability_improvement"] = (
                    site_info["early_percentile_std"] - site_info["late_percentile_std"]
                )
                site_info["volatility_improvement"] = (
                    site_info["early_volatility"] - site_info["late_volatility"]
                )
            ...
            site_info["overall_volatility"] = (
                site_data.sort_values("obs_date")["percentile"].diff().abs().mean()
            )
        ...
        print(
            f"Sites with both early and late data: {self.site_metrics['late_percentile_std'].notna().sum()}"
        )
    ...
    def estimate_stabilization_time(self, stability_threshold=0.8):
        ...
        print("\n" + "=" * 80)
        print(
            f"ESTIMATING STABILIZATION TIME (threshold: {stability_threshold*100:.0f}% of early variability)"
        )
        print("=" * 80)
        ...
        for site_id in self.df["key_plot_id"].unique():
            ...
            site_data_sorted["rolling_std"] = (
                site_data_sorted["percentile"].rolling(window=4, min_periods=2).std()
            )
        ...
        if len(stabilization_df) > 0:
            ...
            print(
                f"                            ({stabilization_df['stabilization_weeks'].median() / 4.33:.1f} months)"
            )
            print(
                f"75th percentile: {stabilization_df['stabilization_weeks'].quantile(0.75):.1f} weeks"
            )
            print(
                f"                 ({stabilization_df['stabilization_weeks'].quantile(0.75) / 4.33:.1f} months)"
            )
            print(
                f"90th percentile: {stabilization_df['stabilization_weeks'].quantile(0.90):.1f} weeks"
            )
            print(
                f"                 ({stabilization_df['stabilization_weeks'].quantile(0.90) / 4.33:.1f} months)"
            )
        else:
    ...
    def analyze_burnin_factors(self):
        ...
        pop_groups = (
            analysis_df.groupby("population_category")
            .agg(
                {
        ...
        # Statistical test
        groups = [
            group["variability_improvement"].dropna().values
        ...
        if len(valid_data) > 10:
            corr, p_val = spearmanr(
                valid_data["population_served"], valid_data["variability_improvement"]
            )
        ...
        state_groups = state_groups.sort_values(
            ("variability_improvement", "mean"), ascending=False
        )
        ...
        duration_groups = (
            analysis_df.groupby("duration_category")
            .agg(
                {
                    "variability_improvement": ["mean", "median", "count"],
                    "stability_improved": "mean",
        ...
        print(
            f"Median variability improvement: {analysis_df['variability_improvement'].median():.2f}"
        )
        ...
        print(
            f"Overall reduction: {((analysis_df['early_percentile_std'].mean() - analysis_df['late_percentile_std'].mean()) / analysis_df['early_percentile_std'].mean() * 100):.1f}%"
        )

        return analysis_df

    def generate_visualizations(
        self, output_dir="/Users/corey/Projects/the-playground/wastewater-disease-prediction/output"
    ):
        ...
        if self.maturity_curves is not None:
            ...
            ax = axes[0, 0]
            ax.plot(
                self.maturity_curves["avg_age_weeks"],
        ...
        if self.site_metrics is not None:
            ...
            if len(analysis_df) > 0:
                ...
                ax = axes[0, 0]
                ax.scatter(
                    analysis_df["early_percentile_std"],
                    analysis_df["late_percentile_std"],
                    alpha=0.5,
                    s=50,
                )
                max_val = max(
                    analysis_df["early_percentile_std"].max(),
                ...
                ax.set_title(
                    "Distribution of Stability Improvement", fontsize=14, fontweight="bold"
                )
                ax.legend()
                ax.grid(True, alpha=0.3)
                ...
                pop_data = [
                    analysis_df[analysis_df["population_category"] == cat][
                        "variability_improvement"
                    ].dropna()
                    for cat in ["<10k", "10k-50k", "50k-100k", "100k-500k", ">500k"]
                ]
                ax.boxplot(
                    [d for d in pop_data if len(d) > 0],
                    labels=[
                        cat
                        for cat, d in zip(
                            ["<10k", "10k-50k", "50k-100k", "100k-500k", ">500k"], pop_data
                        )
                        if len(d) > 0
                    ],
                )
                ...
                ax.set_title(
                    "Burn-in Improvement by Population Size", fontsize=14, fontweight="bold"
                )
                ...
                ax = axes[1, 1]
                ax.scatter(
                    analysis_df["monitoring_days"] / 30.44,
    ...
    def run_full_analysis(self):
        ...
        if len(stabilization_df) > 0:
            ...
            print(
                f"   - Median time to stabilize: {median_weeks:.0f} weeks ({median_weeks/4.33:.1f} months)"
            )
            print(
                f"   - 75% of sites stabilize by: {p75_weeks:.0f} weeks ({p75_weeks/4.33:.1f} months)"
            )
...
def main():
    ...
    if results["stabilization_times"] is not None and len(results["stabilization_times"]) > 0:
        results["stabilization_times"].to_csv(
            f"{output_dir}/burnin_stabilization_times.csv", index=False
        )
