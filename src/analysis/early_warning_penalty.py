...
def get_state_svi_rankings() -> pd.DataFrame:
    ...
    return pd.DataFrame([{"state": k, "svi_score": v} for k, v in state_svi.items()])
...
def get_state_populations() -> pd.DataFrame:
    ...
    return pd.DataFrame([{"state": k, "population_millions": v} for k, v in populations.items()])
...
def load_and_aggregate_state_data(data_dir: Path) -> pd.DataFrame:
    ...
    ww_state = (
        nwss.groupby(["state", "week_end_date"])
        .agg(
            {
                "percentile": "mean",
                "ptc_15d": "mean",
    ...
    ww_state = ww_state.rename(
        columns={"percentile": "ww_percentile", "ptc_15d": "ww_pct_change", "wwtp_id": "n_ww_sites"}
    )
    ...
    merged = pd.merge(ww_state, hosp_state, on=["state", "week_end_date"], how="inner")

    # Calculate total respiratory hospitalizations
    merged["respiratory_total"] = (
        merged["covid_hosp"].fillna(0) + merged["flu_hosp"].fillna(0) + merged["rsv_hosp"].fillna(0)
    )
...
def detect_peaks(
    series: pd.Series, min_prominence: float = 10, min_distance: int = 4
    ...
    peak_indices, properties = signal.find_peaks(
        clean_series.values, prominence=min_prominence, distance=min_distance
    )
...
def calculate_cross_correlation_lag(
    series1: pd.Series, series2: pd.Series, max_lag: int = 8
) -> Tuple[int, float]:
    """
...
def calculate_peak_based_lead_time(
    ww_series: pd.Series, hosp_series: pd.Series, dates: pd.Series
) -> Dict:
    """
...
def calculate_state_lead_times(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for state in df["state"].unique():
        ...
        peak_result = calculate_peak_based_lead_time(
            state_df["ww_percentile"], state_df["covid_hosp"], state_df["week_end_date"]
        )

        # Calculate cross-correlation lead time
        xcorr_lag_weeks, xcorr_corr = calculate_cross_correlation_lag(
            state_df["ww_percentile"], state_df["covid_hosp"], max_lag=8
        )
...
def analyze_early_warning_penalty(lead_times_df: pd.DataFrame, svi_df: pd.DataFrame) -> Dict:
    """
    ...
    merged["svi_quartile"] = pd.qcut(
        merged["svi_score"], q=4, labels=["Q1 (Low)", "Q2", "Q3", "Q4 (High)"]
    )

    # Create coverage quartiles
    merged["coverage_quartile"] = pd.qcut(
        merged["sites_per_100k"], q=4, labels=["Q1 (Low)", "Q2", "Q3", "Q4 (High)"]
    )
    ...
    if valid_peak.sum() >= 10:
        svi_corr, svi_p = stats.pearsonr(
            merged.loc[valid_peak, "svi_score"], merged.loc[valid_peak, "peak_lead_days"]
        )
    ...
    svi_xcorr_corr, svi_xcorr_p = stats.pearsonr(merged["svi_score"], merged["xcorr_lead_days"])
...
def run_early_warning_analysis(data_dir: Path = Path("data")) -> Dict:
    ...
    print(
        f"  States with detectable peaks: {bias['states_with_peaks']} "
    ...
    print(
        f"  SVI vs Lead Time correlation: r = {xcorr['correlation']:.3f}, p = {xcorr['p_value']:.4f}"
    )
    ...
    if "correlation" in results["svi_analysis"]:
        ...
        print(
            f"  SVI vs Lead Time correlation: r = {svi['correlation']:.3f}, p = {svi['p_value']:.4f}"
        )
        ...
        print(
            f"  NOTE: This analysis is biased - peak detection is 2.4x more successful in high-SVI states"
        )
    ...
    if "xcorr_correlation" in results["coverage_analysis"]:
        ...
        print(
            f"  Coverage vs Lead Time (xcorr): r = {cov['xcorr_correlation']:.3f}, p = {cov['xcorr_p_value']:.4f}"
        )
        if "correlation" in cov:
            print(
                f"  Coverage vs Lead Time (peaks): r = {cov['correlation']:.3f}, p = {cov['p_value']:.4f}"
            )
    ...
    if "policy_cost" in results:
        ...
        print(
            f"  Average hospitalizations (high-SVI states): {pc['avg_hospitalizations_high_svi']:.0f}"
        )
        print(f"  Early warning penalty: {pc['penalty_days']:.1f} days")
        print(
            f"  Estimated excess hospitalizations per state: {pc['excess_hosp_low']:.0f}-{pc['excess_hosp_high']:.0f}"
        )
        print(
            f"  Estimated excess percentage: {pc['excess_pct_low']:.1f}%-{pc['excess_pct_high']:.1f}%"
        )
