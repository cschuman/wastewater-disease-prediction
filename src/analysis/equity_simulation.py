...
def create_baseline_dataset(data_dir: Path = Path("data")) -> pd.DataFrame:
    ...
    print(f"Counties with monitoring: {df['has_monitoring'].sum():,} ({df['has_monitoring'].mean()*100:.1f}%)")
    print(f"Population covered: {df['pop_served'].sum()/1e6:.1f}M ({df['pop_served'].sum()/df['population'].sum()*100:.1f}%)")
...
def calculate_current_coverage_by_svi(df: pd.DataFrame) -> pd.DataFrame:
    ...
    stats = df.groupby('svi_quartile').agg({
        'fips': 'count',
        'population': 'sum',
        'has_monitoring': 'sum',
        'n_sites': 'sum',
    ...
    stats.columns = ['SVI Quartile', 'N Counties', 'Total Pop', 'Counties w/ Monitoring',
                     'Total Sites', 'Pop Served', 'Sites per Million']

    # Calculate percentages
    stats['% Counties w/ Monitoring'] = (stats['Counties w/ Monitoring'] / stats['N Counties'] * 100).round(1)
    ...
    # Reorder columns
    stats = stats[[
        'SVI Quartile', 'N Counties', 'Total Pop (M)', 'Counties w/ Monitoring',
        '% Counties w/ Monitoring', 'Total Sites', 'Sites per Million',
        'Pop Served (M)', '% Pop Covered'
    ]]
...
def scenario_a_equalize_per_capita(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict]:
    ...
    quartile_rates = df.groupby('svi_quartile').agg({
        'sites_per_million': 'mean',
        'population': 'sum',
        'n_sites': 'sum'
    }).reset_index()
    ...
    target_rate = quartile_rates.loc[quartile_rates['svi_quartile'] == 'Q1 (Low)', 'sites_per_million'].values[0]
    ...
    df_scenario['new_sites_needed'] = (df_scenario['target_sites'] - df_scenario['n_sites']).clip(lower=0).astype(int)
    ...
    # By SVI quartile
    quartile_summary = df_scenario.groupby('svi_quartile').agg({
        'new_sites_needed': 'sum',
        'population': 'sum',
        'fips': 'count'
    }).reset_index()
    quartile_summary.columns = ['SVI Quartile', 'New Sites Needed', 'Population', 'N Counties']
    quartile_summary['% of New Sites'] = (quartile_summary['New Sites Needed'] / total_new_sites * 100).round(1)
    ...
    state_summary = df_scenario.groupby('state').agg({
        'new_sites_needed': 'sum',
        'population': 'sum',
        'fips': 'count'
    }).reset_index().sort_values('new_sites_needed', ascending=False)

    state_summary.columns = ['State', 'New Sites', 'Population', 'N Counties']
    state_summary['Setup Cost ($M)'] = (state_summary['New Sites'] * COST_PER_SITE_SETUP / 1e6).round(1)
...
def scenario_b_prioritize_zero_coverage(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict]:
    ...
    df_scenario.loc[df_scenario['n_sites'] == 0, 'priority_score'] *= 2  # Double weight for zero coverage

    # Calculate sites needed
    df_scenario['new_sites_needed'] = (df_scenario['min_sites'] - df_scenario['n_sites']).clip(lower=0).astype(int)
    ...
    priority_counties = counties_needing.sort_values('priority_score', ascending=False)[
        ['state', 'county_name', 'population', 'svi_overall', 'n_sites', 'new_sites_needed', 'priority_score']
    ].head(20)

    priority_counties['Population (k)'] = (priority_counties['population'] / 1000).round(0).astype(int)
    priority_counties = priority_counties[[
        'state', 'county_name', 'Population (k)', 'svi_overall', 'n_sites', 'new_sites_needed'
    ]]
    priority_counties.columns = ['State', 'County', 'Pop (k)', 'SVI', 'Current Sites', 'New Sites Needed']

    print(f"\nTop 20 priority counties:")
    print(priority_counties.to_string(index=False))

    # State summary
    state_summary = counties_needing.groupby('state').agg({
        'new_sites_needed': 'sum',
        'fips': 'count',
        'population': 'sum'
    }).reset_index().sort_values('new_sites_needed', ascending=False)
...
def scenario_c_minimum_threshold(df: pd.DataFrame, min_coverage_pct: float = 50.0) -> Tuple[pd.DataFrame, Dict]:
    """
    ...
    df_scenario['additional_pop_needed'] = (df_scenario['target_pop_covered'] - df_scenario['pop_served']).clip(lower=0)
    df_scenario['new_sites_needed'] = np.ceil(df_scenario['additional_pop_needed'] / avg_coverage_per_site).astype(int)
    ...
    print(f"\nCurrent status:")
    print(f"Counties below {min_coverage_pct:.0f}% coverage: {len(counties_below):,} ({len(counties_below)/len(df)*100:.1f}%)")
    ...
    quartile_summary = df_scenario.groupby('svi_quartile').agg({
        'new_sites_needed': 'sum',
        'population': 'sum',
        'fips': lambda x: (counties_below['svi_quartile'] == x.name).sum() if x.name in counties_below['svi_quartile'].values else 0
    }).reset_index()

    quartile_summary.columns = ['SVI Quartile', 'New Sites', 'Population', 'Counties Below Threshold']
    quartile_summary['% of New Sites'] = (quartile_summary['New Sites'] / total_new_sites * 100).round(1)
    ...
    priority_counties = counties_below.sort_values('coverage_gap', ascending=False)[
        ['state', 'county_name', 'population', 'coverage_pct', 'svi_overall', 'n_sites', 'new_sites_needed']
    ].head(20)

    priority_counties['Population (k)'] = (priority_counties['population'] / 1000).round(0).astype(int)
    priority_counties = priority_counties[[
        'state', 'county_name', 'Population (k)', 'coverage_pct', 'svi_overall', 'n_sites', 'new_sites_needed'
    ]]
    priority_counties.columns = ['State', 'County', 'Pop (k)', 'Current Cov %', 'SVI', 'Current Sites', 'New Sites']

    print(f"\nTop 20 counties with largest coverage gaps:")
    print(priority_counties.to_string(index=False))

    # State summary
    state_summary = df_scenario[df_scenario['new_sites_needed'] > 0].groupby('state').agg({
        'new_sites_needed': 'sum',
        'fips': 'count',
        'population': 'sum'
    }).reset_index().sort_values('new_sites_needed', ascending=False)
...
def create_priority_investment_map(df: pd.DataFrame, scenario_results: Dict) -> pd.DataFrame:
    ...
    df_priority['coverage_gap'] = (df_priority['expected_sites'] - df_priority['n_sites']).clip(lower=0)
    df_priority['score_gap'] = (df_priority['coverage_gap'] / df_priority['coverage_gap'].max() * 100).fillna(0)

    # Component 3: Population impact (0-100, log scale for fairness)
    df_priority['score_population'] = (np.log10(df_priority['population']) / np.log10(df_priority['population'].max()) * 100)
    ...
    investment_map = top_priorities[[
        'state', 'county_name', 'population', 'svi_overall', 'svi_quartile',
        'n_sites', 'coverage_pct', 'priority_score', 'priority_tier'
    ]].copy()
    ...
    investment_map = investment_map[[
        'state', 'county_name', 'Population (k)', 'svi_overall', 'svi_quartile',
        'n_sites', 'coverage_pct', 'Priority Score', 'priority_tier'
    ]]

    investment_map.columns = [
        'State', 'County', 'Pop (k)', 'SVI', 'SVI Quartile',
        'Current Sites', 'Coverage %', 'Priority Score', 'Tier'
    ]
    ...
    # Tier summary
    tier_summary = df_priority.groupby('priority_tier').agg({
        'fips': 'count',
        'population': 'sum',
        'n_sites': 'sum'
    }).reset_index()
    ...
    tier_summary['% of US Population'] = (tier_summary['Population'] / tier_summary['Population'].sum() * 100).round(1)
...
""")
    ...
    for idx, row in top_states.iterrows():
        print(f"  {idx+1}. {row['State']}: {row['New Sites']:,} sites, ${row['Setup Cost ($M)']:.0f}M")
...
def run_full_equity_simulation(data_dir: Path = Path("data")) -> Dict:
    ...
    df_priority[['fips', 'state', 'county_name', 'population', 'svi_overall', 'svi_quartile',
                 'n_sites', 'coverage_pct', 'priority_score', 'priority_tier']].to_csv(
        output_dir / 'county_priority_rankings.csv', index=False
    )
