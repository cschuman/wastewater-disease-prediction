...
def create_priority_county_heatmap(priority_file: Path, output_dir: Path):
    ...
    ax.set_yticklabels([f"{row['State']} - {row['County'][:25]}" for _, row in df.iterrows()], fontsize=9)
    ax.set_xlabel('Social Vulnerability Index (SVI)', fontsize=12, fontweight='bold')
    ax.set_title('Top 30 Priority Counties for Wastewater Surveillance Investment',
    ...
    legend = ax.legend(legend_bubbles, legend_labels,
...
def create_state_investment_map(priority_file: Path, output_dir: Path):
    ...
    axes[1].set_xlabel('Total Population in Priority Counties (thousands)', fontsize=12, fontweight='bold')
    axes[1].set_ylabel('Current Monitoring Sites', fontsize=12, fontweight='bold')
    axes[1].set_title('Current Coverage vs Population (Top 20 States)', fontsize=14, fontweight='bold')
...
def create_coverage_gap_analysis(rankings_file: Path, output_dir: Path):
    ...
    quartile_sites = df.groupby('svi_quartile').agg({
        'n_sites': 'sum',
        'population': 'sum'
    }).reset_index()
    ...
    axes[0, 0].bar(quartile_sites['svi_quartile'], quartile_sites['sites_per_million'],
                   color=colors_q, edgecolor='black', linewidth=1.5)
    axes[0, 0].set_ylabel('Sites per Million Population', fontsize=11, fontweight='bold')
    axes[0, 0].set_title('Current Monitoring Density by SVI Quartile', fontsize=13, fontweight='bold')
    ...
    pct_monitoring = df.groupby('svi_quartile').agg({
        'has_monitoring': 'mean'
    }).reset_index()
    pct_monitoring['pct'] = pct_monitoring['has_monitoring'] * 100

    axes[0, 1].bar(pct_monitoring['svi_quartile'], pct_monitoring['pct'],
                   color=colors_q, edgecolor='black', linewidth=1.5)
    ...
    axes[1, 0].axvline(df['priority_score'].median(), color='red', linestyle='--',
                       linewidth=2, label=f'Median: {df["priority_score"].median():.1f}')
    ...
    axes[1, 0].set_title('Distribution of Priority Scores Across All Counties',
    ...
    axes[1, 1].set_title('Coverage vs Vulnerability (Sample of Counties)',
                         fontsize=13, fontweight='bold')
...
def create_executive_summary_viz(scenario_file: Path, rankings_file: Path, output_dir: Path):
    ...
    # Title
    fig.suptitle('Wastewater Surveillance Equity Simulation: Executive Summary',
    ...
    quartile_stats.plot(kind='bar', ax=ax1, color=['#2E86AB', '#A8DADC', '#F18F01', '#A23B72'],
    ...
    ax2.bar(x - width/2, scenarios['Setup Cost ($M)'], width,
            label='Setup Cost', color='#2E86AB', edgecolor='black', linewidth=1.5)
    ax2.bar(x + width/2, scenarios['5-Year Cost ($M)'] - scenarios['Setup Cost ($M)'], width,
            bottom=scenarios['Setup Cost ($M)'], label='5-Year Operating',
    ...
    ax3.barh(scenarios['Scenario'], scenarios['New Sites'],
    ...
    tier_counts.plot(kind='bar', ax=ax4, color=colors_tier[:len(tier_counts)],
                    edgecolor='black', linewidth=1.5)
    ...
    top_states = rankings.nlargest(50, 'priority_score').groupby('state')['priority_score'].mean().nlargest(10)
    ...
    high_svi_no_coverage = rankings[(rankings['svi_quartile'] == 'Q4 (High)') &
                                     (rankings['has_monitoring'] == 0)].shape[0]
