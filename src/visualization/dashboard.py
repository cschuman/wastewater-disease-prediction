...
def create_time_series_plot(
    ...
    if state != "US":
        ...
        df = df.groupby('week_end_date').agg({
            'covid_hosp': 'sum',
            'flu_hosp': 'sum',
            'rsv_hosp': 'sum',
...
def create_forecast_plot(
    ...
    if state != "US":
        ...
        fcast = forecast_df[(forecast_df['state'] == state) & (forecast_df['target'] == target)].copy()
    else:
        hist = historical_df.groupby('week_end_date').agg({target: 'sum'}).reset_index()
        fcast = forecast_df[forecast_df['target'] == target].groupby('target_date').agg({
            'point_estimate': 'sum',
            'lower_bound': 'sum',
            'upper_bound': 'sum'
        }).reset_index()
    ...
    if len(backtest_results) > 0:
        mape = np.mean(np.abs((backtest_results['actual'] - backtest_results['predicted']) / backtest_results['actual'])) * 100
        title = f'{target.replace("_", " ").title()} Forecast - {state} (Backtest MAPE: {mape:.1f}%)'
    else:
...
def _generate_backtest_predictions(
    ...
    feature_cols = [f'{target}_lag1', f'{target}_lag2', f'{target}_lag3',
                    f'{target}_roll2', 'pct_change', 'week_of_year', 'month']
...
def create_full_dashboard(
    ...
    forecast_fig = create_forecast_plot(featured, forecast_df, target='respiratory_total', state="US")
