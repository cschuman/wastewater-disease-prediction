...
def load_and_merge_data(data_dir: Path) -> pd.DataFrame:
    ...
    covid_ww["week_end"] = (
        pd.to_datetime(covid_ww["date_end"]).dt.to_period("W-SAT").dt.end_time.dt.normalize()
    )
    covid_ww["pop_weighted_percentile"] = covid_ww["percentile"] * covid_ww["population_served"]

    covid_agg = (
        covid_ww.groupby(["state", "week_end"])
        .agg(
            {
                "percentile": "mean",
    ...
    covid_agg["covid_ww_percentile"] = (
        covid_agg["pop_weighted_percentile"] / covid_agg["population_served"]
    )
    covid_agg = covid_agg.rename(
        columns={
    ...
    # Aggregate to state-week
    flu_ww["week_end"] = (
        pd.to_datetime(flu_ww["sample_collect_date"])
        .dt.to_period("W-SAT")
        .dt.end_time.dt.normalize()
    )

    flu_agg = (
        flu_ww.groupby(["state", "week_end"])
        .agg(
            {
    ...
    merged = pd.merge(
        merged, hosp_clean, on=["state", "week_end_date"], how="inner", suffixes=("", "_hosp")
    )
...
def create_multi_pathogen_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for state in df["state"].unique():
        ...
        state_df["week_of_year"] = (
            pd.to_datetime(state_df["week_end_date"]).dt.isocalendar().week.astype(int)
        )
...
def build_xgboost_model(
    ...
    if len(train_clean) < 50 or len(test_clean) < 10:
        logger.warning(
            f"Insufficient data for {target_col}: train={len(train_clean)}, test={len(test_clean)}"
        )
        return None
...
def run_multi_pathogen_experiment(data_dir: Path) -> pd.DataFrame:
    ...
    for target, use_covid, use_flu, desc in experiments:
        ...
        if result:
            ...
            print(
                f"{desc:25s} MAE: {result['mae']:8.2f}  RMSE: {result['rmse']:8.2f}  MAPE: {result['mape']:5.1f}%"
            )
    ...
    for target in ["covid_hosp", "flu_hosp", "rsv_hosp", "respiratory_total"]:
        ...
        if len(target_results) > 0:
            ...
            no_ww = target_results[~target_results["COVID_WW"] & ~target_results["Flu_WW"]][
                "MAE"
            ].values
            with_ww = target_results[target_results["COVID_WW"] | target_results["Flu_WW"]][
                "MAE"
            ].min()
    ...
    print("=" * 70)
    print(
        results_df.sort_values("MAE")[["Description", "MAE", "RMSE", "MAPE"]].to_string(index=False)
    )
